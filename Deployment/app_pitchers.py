...
st.sidebar.markdown("#### Built by Muhammad Rizky Azzakky")
...
# 2024 pitching dataframe
pitching_2024_df = pd.read_csv('pitching_merged_2024', index_col = 0)

# reformat 2024 pitching df for model prediction
df_to_predict = pitching_2024_df.drop(columns = ['Name', '2024 Salary'])

# load in model
pb_model = joblib.load("pb_model.pkl")

# make prediction
predictions_2024 = pb_model.predict(df_to_predict)

# Add prediction column
pitching_2024_df["Predicted Salary"] = np.around(np.exp(predictions_2024),0)

# Add value column
pitching_2024_df.loc[pitching_2024_df['Predicted Salary'] > pitching_2024_df['2024 Salary'], 'Value?'] = 'Under-valued'
pitching_2024_df.loc[pitching_2024_df['Predicted Salary'] < pitching_2024_df['2024 Salary'], 'Value?'] = 'Over-valued'

# reorder columns
pitching_2024_df= pitching_2024_df[['Name', '2024 Salary', 'Predicted Salary', 'Value?', 'Avg Career Salary Difference', 'Age', \
                                'W', 'L', 'ERA', 'G', 'SV', 'IP', 'H', 'HR', 'SO', 'BB']]

# formatting as Millions
pitching_2024_df['2024 Salary']  = pitching_2024_df['2024 Salary'] .div(1000000).round(2)
pitching_2024_df['Predicted Salary'] = pitching_2024_df['Predicted Salary'] .div(1000000).round(2)
pitching_2024_df['Avg Career Salary Difference']  = pitching_2024_df['Avg Career Salary Difference'].div(1000000).round(2)

pitching_2024_df = pitching_2024_df.rename(columns = {'2024 Salary':'2024 Salary ($ Millions)',
                                                  'Predicted Salary':'Predicted Salary ($ Millions)',
                                                  'Avg Career Salary Difference':'Avg Career Salary Difference ($ Millions)'})


st.dataframe(pitching_2024_df)
